...
class KeepKeySigningProtocolTest {
    ...
    @Test
    fun zeroActionsReturnsEmptySignatureList() =
        runBlocking {
            val transport =
                FakeTransport(
    ...
    @Test
    fun zeroActionsOnlyOneMessageSent() =
        runBlocking {
            val transport =
                FakeTransport(
                    listOf(MSG_ZCASH_SIGN_PCZT to Pair(MSG_ZCASH_PCZT_ACTION_ACK, ackBytes(0)))
    ...
    @Test
    fun singleActionCollectsSignature() =
        runBlocking {
    ...
    @Test
    fun twoActionsWithIntermediateAck() =
        runBlocking {
    ...
    @Test
    fun correctAccountIndexIsSentInInitRequest() =
        runBlocking {
            val transport =
                FakeTransport(
                    listOf(MSG_ZCASH_SIGN_PCZT to Pair(MSG_ZCASH_PCZT_ACTION_ACK, ackBytes(0)))
            ...
            val sent =
                com.keepkey.deviceprotocol.KeepKeyMessageZcash.ZcashSignPCZT
                    .parseFrom(transport.sentPayloads[0])
            assertEquals(7, sent.account)
        }

    @Test
    fun pcztBytesAreSentVerbatim() =
        runBlocking {
